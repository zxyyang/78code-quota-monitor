...
async function apiLogin(username, password) {
  const res = await httpReq({
    url: 'https://9527code.com/api/user/login?turnstile=',
    method: 'POST',
    headers: {
...
async function apiGetUser(session, userId) {
  ...
  const res = await httpReq({ url: 'https://9527code.com/api/user/self', headers });
...
async function apiGetTokens(session, userId) {
  ...
  if (userId) headers['new-api-user'] = String(userId);
  const res = await httpReq({ url: 'https://9527code.com/api/token/?p=1&size=100', headers });
  return { status: res.status, data: JSON.parse(res.body) };
}
...
async function apiGetGroups(session, userId) {
  ...
  if (userId) headers['new-api-user'] = String(userId);
  const res = await httpReq({ url: 'https://9527code.com/api/user/self/groups', headers });
  return { status: res.status, data: JSON.parse(res.body) };
}
...
async function apiGetSubscription(session, userId) {
  ...
  if (userId) headers['new-api-user'] = String(userId);
  const res = await httpReq({ url: 'https://9527code.com/api/subscription/self', headers });
  return { status: res.status, data: JSON.parse(res.body) };
}
