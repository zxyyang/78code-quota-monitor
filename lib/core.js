#!/usr/bin/env node
// 9527code Quota Monitor - 核心 API 和工具模块
...
// 旧版注入标记（用于清理兼容）
const START_MARKER = '// [9527code-quota-start]';
const END_MARKER = '// [9527code-quota-end]';
...
function httpFetch(url) {
  return new Promise((resolve, reject) => {
    const u = new URL(url);
    const r = https.request({ hostname: u.hostname, path: u.pathname + u.search, method: 'GET' }, res => {
      let d = '';
      res.on('data', c => d += c);
...
async function apiLogin(username, password) {
  const res = await httpReq({
    url: 'https://api.9527code.com/api/user/login?turnstile=',
    method: 'POST',
    headers: {
...
async function apiGetUser(session, userId) {
  ...
  const res = await httpReq({ url: 'https://api.9527code.com/api/user/self', headers });
...
async function apiGetTokens(session, userId) {
  ...
  if (userId) headers['new-api-user'] = String(userId);
  const res = await httpReq({ url: 'https://api.9527code.com/api/token/?p=1&size=100', headers });
  return { status: res.status, data: JSON.parse(res.body) };
}

function detectCurrentGroup(tokens) {
  if (!tokens || tokens.length === 0) return null;
  // 读取环境变量或 settings 中的 API key（兼容 ANTHROPIC_API_KEY 和 ANTHROPIC_AUTH_TOKEN）
  ...
  if (!apiKey) {
    try {
      ...
      apiKey = (settings.env && (settings.env.ANTHROPIC_API_KEY || settings.env.ANTHROPIC_AUTH_TOKEN)) || '';
    } catch (e) {}
  }
...
async function apiGetGroups(session, userId) {
  ...
  if (userId) headers['new-api-user'] = String(userId);
  const res = await httpReq({ url: 'https://api.9527code.com/api/user/self/groups', headers });
  return { status: res.status, data: JSON.parse(res.body) };
}
...
async function apiGetSubscription(session, userId) {
  ...
  if (userId) headers['new-api-user'] = String(userId);
  const res = await httpReq({ url: 'https://api.9527code.com/api/subscription/self', headers });
  return { status: res.status, data: JSON.parse(res.body) };
}

function formatQuota(val) {
  if (val == null || isNaN(val)) return '?';
  return (val / 500000).toFixed(2);
}
...
function installStatusline() {
  ...
  // 安装后自动触发一次后台 check 刷新缓存
  try {
    const { spawn } = require('child_process');
    const checkScript = path.join(MONITOR_DIR, 'check.js');
    if (fs.existsSync(checkScript) && fs.existsSync(CONFIG_FILE)) {
...
module.exports = {
  ...
  INTERVALS,
  ensureDir, readConfig, writeConfig, readCache, writeCache, log, httpFetch,
