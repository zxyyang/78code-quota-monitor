...
async function autoUpdate() {
  ...
  try {
    ...
    if (fs.existsSync(pkgPath)) {
      ...
      try {
        const out = execSync('npm list -g 9527code-quota --json', { timeout: 10000, encoding: 'utf8', stdio: ['pipe', 'pipe', 'ignore'] });
        currentVersion = JSON.parse(out).dependencies['9527code-quota'].version;
      } catch (e) {
        currentVersion = null;
      }
    }
    if (!currentVersion) return;
    const result = await core.httpFetch('https://registry.npmjs.org/9527code-quota/latest');
    const latest = JSON.parse(result).version;

    if (latest && latest !== currentVersion) {
      core.log(`AUTO-UPDATE: ${currentVersion} -> ${latest}`);
      execSync('npm install -g 9527code-quota@latest', { stdio: 'ignore', timeout: 60000 });
      // 重新读取最新 core 并同步脚本
