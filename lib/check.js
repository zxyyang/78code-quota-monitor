...
async function autoUpdate() {
  ...
  try {
    ...
    if (fs.existsSync(pkgPath)) {
      ...
      try {
        const out = execSync('npm list -g 78code-quota-monitor --json', { timeout: 10000, encoding: 'utf8', stdio: ['pipe', 'pipe', 'ignore'] });
        currentVersion = JSON.parse(out).dependencies['78code-quota-monitor'].version;
      } catch (e) {
        currentVersion = null;
      }
    }
    if (!currentVersion) return;
    const result = await core.httpFetch('https://registry.npmjs.org/78code-quota-monitor/latest');
    const latest = JSON.parse(result).version;

    if (latest && latest !== currentVersion) {
      core.log(`AUTO-UPDATE: ${currentVersion} -> ${latest}`);
      execSync('npm install -g 78code-quota-monitor@latest', { stdio: 'ignore', timeout: 60000 });
      // 重新读取最新 core 并同步脚本
